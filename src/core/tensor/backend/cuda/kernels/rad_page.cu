...
namespace lfs::core::internal {
    namespace {
        ...
        using lfs::core::RadPackedProperty;
        ...
        __global__ void lodPageDequantKernel(const std::uint8_t* const __restrict__ slot,
            ...
            if (i == 0u) {
                ...
                float4* const out = pool.page_frames +
                                    static_cast<std::size_t>(page) * radq::kPageFrameFloat4s;
                for (std::uint32_t f = 0; f < radq::kPageFrameFloat4s; ++f) {
                    out[f] = src[f];
                }
            }
            ...
            if (const auto* const prop = plane_of(RadPackedKind::Scales); pool.scaling != nullptr) {
                ...
                if (live && prop != nullptr) {
                    if (static_cast<RadPackedEncoding>(prop->encoding) == RadPackedEncoding::LnF16) {
                        // Log-domain f16 payloads pass through without rounding.
                        for (std::uint32_t d = 0; d < 3u; ++d) {
                            memcpy(&h[d],
        ...
        __device__ float readResidentSh(const LodPageTensorSources& src,
            ...
            static_assert(kReorder == 32u);
            const auto index = sh::offset(splat, c, src.src_rest, false);
            return src.shN_f16
        ...
        __global__ void lodPageQuantizeKernel(const LodPageTensorSources src,
            ...
            const float4* const frame =
                pool.page_frames + static_cast<std::size_t>(page) * radq::kPageFrameFloat4s;
    ...
    cudaError_t launchLodPageQuantizeFromTensors(const LodPageTensorSources& src,
        ...
        if (src.shN != nullptr && src.src_rest > 0u) {
