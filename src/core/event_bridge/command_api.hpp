...
namespace lfs::training {
    ...
    public:
        ...
        LFS_BRIDGE_API void overlay_stored_session(std::string strategy, bool hydrated);
        LFS_BRIDGE_API void bind_state_events();
        LFS_BRIDGE_API void clear_snapshot(const Trainer* trainer);
        // Drop trainer-owned snapshot fields after teardown (no trainer loaded).
        LFS_BRIDGE_API void reset_snapshot();

        [[nodiscard]] LFS_BRIDGE_API TrainingSnapshot snapshot() const;
        [[nodiscard]] LFS_BRIDGE_API std::vector<LossHistoryPoint> loss_history() const;
        LFS_BRIDGE_API void clear_loss_history();
        LFS_BRIDGE_API void replace_loss_history(
