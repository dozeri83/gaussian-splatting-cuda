...
TEST(ArgumentParserTest, EvalStepsWithoutEvaluationStopTheRun) {
    ...
    for (const char* flag : {"--eval-steps", "--eval-steps=7000,20000"}) {
        ...
        auto parsed = lfs::io::args::parse_args_and_params(static_cast<int>(argv.size()), argv.data());
    ...
    auto parsed = lfs::io::args::parse_args_and_params(static_cast<int>(std::size(with_eval)), with_eval);
    ASSERT_TRUE(parsed.has_value()) << parsed.error();
